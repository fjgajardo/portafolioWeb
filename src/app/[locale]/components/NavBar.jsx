...
const NavBar = () => {
    ...
    return (
        <nav className='py-8 flex justify-between px-6 md:px-36 z-10 bg-blancoBlanco sticky top-0 border-b-2 border-blanco'>
          ...
          <ul className={`${nav ? 'flex' : 'hidden'}  md:flex md:flex-row md:static md:top-0 md:rounded-none md:h-auto md:shadow-none md:w-auto md:px-0 
          bg-blancoBlanco  flex-col align-right mr-3 items-right text-right absolute  top-20  right-0 w-64 shadow-md  z-20 my-4 py-3 md:pb-0 rounded-md top-0 h-screen`}>
            {links.map((link) => (
              <li key={link.toString()} className='mx-4 md:mx-4 hover:border-b-2 border-verde ease-linear block md:inline-block my-2 md:my-0 text-gris font-semibold text-lg md:text-base i-right '>
